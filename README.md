...
## 플레이어 공격 모션 실행
슈팅게임제작시 MuzzleEffect나 총 사운드도 복제 변수처럼 변수가 변경되었을 때 지정된 함수를 이용하여 실행할 수 있긴 하다.
그러나 이러한 이펙트 요소들은 게임에 있으면 좋지만 없다고 해서 게임 플레이에 영향이 가지는 않으며, 일회성으로 한번 실행하고 나면 그만인 행동들이다.
따라서 굳이 변수를 복제하여 상태를 바꾸고 이펙트를 실행 후, 다시 이펙트를 실행할 수 있게끔 변수를 변경하고 복제하는 것은 다소 비효율적이다.
