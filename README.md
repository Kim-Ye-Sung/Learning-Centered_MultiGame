...
예를들어 클라이언트 플레이어가 앉기 상태로 돌입하여 속도를 낮추고 싶다면 서버와 다른 클라이언트쪽에서도 그 클라이언트 캐릭터의 속도가 낮춰져야 한다.  
따라서 변수가 복제될수 있게 설정하고, 그 변수가 서버쪽에서 바뀜으로 인해서 다른 클라이언트들도 모두 똑같이 복제되어 적용될 수 있도록 한다.  

1. 직접 만들어둔 액터 컴포넌트인 CharacterState에서 상태 변수를 복제되게끔 설정한다.
이때, 단순 복제가 아니라 ReplicatedUsing을 사용하여 변수가 복제될때 클라이언트측에서 자동으로 실행될 함수를 지정해준다.
...
## 플레이어 공격 모션 실행
슈팅게임제작시 MuzzleEffect나 총 사운드도 복제 변수처럼 변수가 변경되었을 때 지정된 함수를 이용하여 실행할 수 있긴 하다.<br>
그러나 이러한 이펙트 요소들은 게임에 있으면 좋지만 없다고 해서 게임 플레이에 영향이 가지는 않으며, 일회성으로 한번 실행하고 나면 그만인 행동들이다.<br>
따라서 굳이 변수를 복제하여 상태를 바꾸고 이펙트를 실행 후, 다시 이펙트를 실행할 수 있게끔 변수를 변경하고 복제하는 것은 다소 비효율적이다.<br>
이러한 경우는 서버RPC와 NetMulticastRPC를 조합하여 일회성으로 이펙트를 한번만 실행하고 끝내는 것이 효율적이다.<br>
*** 설명은 이펙트 관련하여 말하였지만 결과를 좀 더 명확하게 볼 수 있도록 근접공격 애니메이션을 사용하여 NetMulticast를 실행한다. ***

1. 근접 공격 애니메이션을 실행할 NetMulticastRPC를 선언하고 정의한다.  
   근접공격은 공격과 관련한 중요한 모션이기에 Reliable로 선언해주었지만  
...
6. 플레이어측에서 상호작용을 위한 라인트레이스 함수를 선언 및 정의한다.
   ...
   인터페이스를 구현한 오브젝트라면 5번 과정에서 정의하였던 서버RPC를 실행하여 오브젝트가 가진 인터페이스 함수를 실행한다. 
...
1. 스킬 키를 누르면 칼을 뽑는 몽타주를 재생하고, 스킬 시전 시간을 타이머로 잰다.<br>
타이머가 끝난 후에는 스킬과 관련한 기능을 초기화하고 칼을 다시 넣는 몽타주를 재생한다.
...
<결과><br>
스킬 키를 누르면 등에 달려있던 칼이 자연스럽게 손으로 옮겨가며, 공격키를 누를때마다 횡베기와 종베기를 번갈아가면서 실행하는 것을 확인할 수 있다.<br>
또한 적 오브젝트가 피해를 입을 시 "TakeDamage"라는 문구가 나오는 것을 확인할 수 있으며, 한번 베었던 적은 다시 데미지를 입지 않아서 
한번 휘두를때마다 오브젝트 하나당 "TakeDamage"라는 문구가 하나만 나오는 것을 확인할 수 있다.<br>
